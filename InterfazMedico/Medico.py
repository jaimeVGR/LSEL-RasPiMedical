...
class Medico:
	...
	def __init__(self):
		"""
			Constructor donde se encuentra guardada la clave de lectura de Thingspeak y la clave del código del canal. 
			Adicionalmente se inicializan los vectores donde se van a guardar los datos obtenidos desde el servidor. 
		"""
		self.code = 480292
	...
	def getData(self, identificador, etiqueta): 
		"""
			Método empleado para devolver los datos obtenidos desde Thingspeak
		"""
		mean = []
		desv = []
	...
	def showGraph(self, data, codigoGrafico, etiqueta):
		"""
			Ḿétodo empleado para mostrar la gráfica
		""" 
		minimo = data[0]
		maximo = data[0]
	...
	def updateFile(self, name): 
		"""
			Inserta Pacientes en la base de datos
		"""
		f = open("Pacientes.txt", "a")	
	...
	def findUser(self, user): 
		"""
			Busca el usuario en la BBDD y devuelve su identificador
		"""
